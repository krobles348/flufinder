upload_fasta <- function(testfasta_file) {
  
  library(seqinr)
  
  read.fasta(testfasta_file,
             seqtype = "AA",
             as.string = TRUE,
...
splitpeptides_to_masses <- function(aa) {
  ...
  aa_masses <- c(
    A=71.037, R=156.101, N=114.042, D=115.026, C=103.009,
...
ggbarplot <- function(peptide_counts_table) {
  ...
  ggplot(peptide_counts_table) +
    aes(x = rownames(peptide_counts_table),
        y = peptide_counts) +
